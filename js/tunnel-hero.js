// Wormhole "Tunnel" scene, scoped to the homepage hero as a decorative background.
// Adapted from a full-page Three.js recreation: geometry, shaders and uniforms are
// unchanged; sizing, scroll progress, pointer tracking and the render loop are scoped
// to the hero container instead of the whole document.
...
if (heroEl && canvas && window.matchMedia('(prefers-reduced-motion: reduce)').matches === false) {
  initTunnel(heroEl, canvas);
}

function initTunnel(heroEl, canvas) {
  // ---------- helpers ----------
  ...
  // ---------- sizing (scoped to the hero container) ----------
  function getSize() {
...
`;
  ...
  const points = new THREE.Points(tunnelGeometry, tunnelMaterial);
...
`
  ...
  // ---------- resize (scoped to hero container) ----------
  function onResize() {
    const s = getSize();
    W = s.w; H = s.h;
  ...
  if ('ResizeObserver' in window) {
    new ResizeObserver(onResize).observe(heroEl);
  }

  // ---------- scroll: progress through the hero's own scroll range ----------
  let scrollTarget = 0, scrollSmooth = 0, scrollCurrent = 0;
  function onScroll() {
    const rect = heroEl.getBoundingClientRect();
    const range = Math.max(1, rect.height);
    scrollTarget = clamp(-rect.top / range, 0, 1);
  }
  window.addEventListener('scroll', onScroll, { passive: true });
  onScroll();

  // ---------- pointer (tracked relative to the hero container) ----------
  ...
  heroEl.addEventListener('mouseleave', () => { POINTER.active = false; });
