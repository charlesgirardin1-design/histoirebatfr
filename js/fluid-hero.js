// Fluid-ink hero: staggered word/block reveal on load, plus a full-bleed
// WebGL fluid simulation (oily ink-in-water) behind the hero content.
// The simulation engine is adapted from Pavel Dobryakov's WebGL fluid sim (MIT).
(function () {
  "use strict";
  ...
  function revealWords(el, spans, baseDelay, stagger) {
    if (!el) return;
    if (reduceMotion) { el.classList.add("revealed"); return; }
  ...
  var headingWords = wrapWords(heading);
  var ledeWords = wrapWords(lede);

  reveal(document.getElementById("heroBadge"), 320);
  revealWords(heading, headingWords, 480, 85);
  revealWords(lede, ledeWords, 1150, 22);
  reveal(document.getElementById("heroActions"), 1450);
  reveal(document.getElementById("heroStats"), 1550);
  reveal(document.getElementById("heroScrollHint"), 1650);
  ...
  if (!reduceMotion) {
    ...
    if (canvas && (window.WebGLRenderingContext || window.WebGL2RenderingContext)) {
      try { fluidSimulation(canvas); } catch (e) { /* WebGL unsupported or blocked: hero stays a plain dark gradient */ }
    }
  }
})();
