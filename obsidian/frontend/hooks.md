...
| `useReducedMotionPreference` | `use-reduced-motion-preference.ts` | Live `prefers-reduced-motion` boolean for render loops outside react-spring (e.g. an `@react-three/fiber` `useFrame` loop) — `<ReducedMotion>` only reaches react-spring's own `skipAnimation`. Powers [[components/ui|`<CubeAnimation>`]]. |
