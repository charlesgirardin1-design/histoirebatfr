...
## ADR-0023 — Authorised engine fix: `createElement` over polymorphic JSX in `AnimatedVarTextTag`

**Status:** Accepted · 2026-08-24 · authorised by explicit user sign-off, per ADR-0009

**Decision.** In the five `#do-not-modify` files that define a local
`AnimatedVarTextTag`/`VarTextTag` (`animated-var-text-tag.tsx`, `hover.tsx`,
`progress-trigger.tsx`, `spring-trigger.tsx` [two instances], `spring.tsx`),
swap the polymorphic JSX return —
`<Tag ref={ref} className={className} style={style} {...props}>{children}</Tag>` —
for `createElement(Tag, { ref, className, style, ...props }, children)`. Purely
mechanical: same props object, same children, same runtime output.

**Why.** Adding `three` / `@react-three/fiber` ([[tech-stack]] → "3D") for
[[components/ui|`<CubeAnimation>`]] broke `yarn build`'s type-check in these
five files with `TS2745`/`TS2322` ("children prop expects type 'never'"),
though `yarn dev` and `yarn lint` stayed clean and a pristine baseline build
(verified in a throwaway worktree at the initial commit) did not reproduce it.
Root cause: `@react-three/fiber` globally augments `JSX.IntrinsicElements`
(`declare module 'react'|'react/jsx-runtime'|'react/jsx-dev-runtime'`) with
`ThreeElements` — a large, ambient, project-wide merge with no per-file
opt-out. A JSX tag typed as a bare `ElementType`/`React.ElementType` (as
`animated[tag] as ElementType` here) gets checked against **every** member of
that now much bigger `IntrinsicElements` map; some members disallow children
entirely, and the merged expected type collapses to `never`.
`React.createElement`'s `ElementType` overload does not route through
`JSX.IntrinsicElements` at all, so it sidesteps the collision without
touching the components' actual behaviour.

**When building.** This is **not** a precedent for editing the engine further
(same caveat as ADR-0009) — each future touch needs its own explicit sign-off.
If another polymorphic-tag JSX pattern surfaces the same `never`-collapse
error after adding a library that globally augments `JSX.IntrinsicElements`,
the same `createElement` swap is the fix — do not add `@ts-expect-error` or
loosen the prop types instead, and do not attempt to scope the augmentation
away (TypeScript's `declare module` merges are program-wide by construction;
there is no tsconfig-level opt-out).
