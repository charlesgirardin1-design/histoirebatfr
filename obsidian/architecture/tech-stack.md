...
## 3D (added per project — not in the starter baseline)

| Package | Version | Role |
|---------|---------|------|
| `three` | `^0.185.1` | WebGL engine |
| `@react-three/fiber` | `^9.7.0` | React renderer for `three` |
| `@react-three/drei` | `^10.7.8` | `three`/fiber helper primitives (`RoundedBox`, `Edges`, `Environment`, …) |
| `@types/three` | `^0.185.4` | Type definitions (dev) — `three` does not ship its own |
| `simplex-noise` | `^4.0.3` | 2D/3D simplex noise — drives `<ScrollTerrain>`'s vertex heights |

Added for `<CubeAnimation>` and `<ScrollTerrain>` ([[components/ui]]). A WebGL render loop (`useFrame`)
is not DOM/React motion and is not governed by hard rule #1 — see
[[optimize-3d-scene]] → "How it sits with the hard rules". If a future scene
needs a performance pass, that workflow is the source of truth for the order of
fixes — don't improvise one.
...
| 3D / WebGL scene | `three` + `@react-three/fiber` + `@react-three/drei` | [[optimize-3d-scene]] |
